...
extract($_REQUEST);
 ...
 $fullbook="";
$id_fasiliti = $_REQUEST['id_fasiliti'];
$tarikh_mula = date('Y-m-d', strtotime($_POST['trkmula'])); 
$tarikh_tamat = date('Y-m-d', strtotime($_POST['trktamat'])); 
	while (strtotime($tarikh_mula) <= strtotime($tarikh_tamat)) {

		
	$sehari=mysql_num_rows(mysql_query("select * from add_fasiliti where tarikh = '$tarikh_mula' AND id_fasiliti = '$id_fasiliti' AND masa='2PM-12PM' AND status_approve = 2"));
	...
	echo "<script type='text/javascript'>";
		echo "alert('Tempahan Asrama Anda Pilih Sudah Penuh. Sila pilih tarikh lain.')";
		echo "</script>";
		echo "<meta http-equiv='refresh' content='0;url=tempahan_asrama_mula.php?id_fasiliti=$id_fasiliti'>";
...
<input type="hidden" name="id_fasiliti" value="<?=$_POST['id_fasiliti'];?>">
  <?php
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="hovertable2" style="padding:2px;">
  <tr>
    ...
    <td width="1%">:</td>
    <td width="81%">
	<?php $trk_mula = $_POST['trkmula']; $n_trk_mula = date('d-m-Y', strtotime($trk_mula)); echo $n_trk_mula;?>
	...
	<input type="hidden" name="kategori" value="<?php echo $cat; ?>" />
  	<input type="hidden" name="fasiliti" value="<?php echo $nama; ?>" />
	  	<input type="hidden" name="t_mula" value="<?php echo $n_trk_mula; ?>" />
    </td>
    </tr>
  <tr>
    <td>Tarikh Tamat</td>
    <td>:</td>
    <td>
	<?php $trk_tamat = $_POST['trktamat']; $n_trk_tamat = date('d-m-Y', strtotime($trk_tamat)); echo $n_trk_tamat;?>
    <input type="hidden" name="date_end" value="<?php echo $trk_tamat; ?>" />
		  	<input type="hidden" name="t_akhir" value="<?php echo $n_trk_tamat; ?>" />
    </td>
    </tr>
  <tr>
    <td>Jumlah Hari</td>
    ...
    <td valign="top"><select size="1" name="jenis_susunan">
      <?php
				...
				for($x=0;$x<3;$x++)
				{
					if($x > 0)
					{
						echo "<option value='$cat[$x]'>$cat[$x]</option>";
					}
				}
			?>
    </select></td>
