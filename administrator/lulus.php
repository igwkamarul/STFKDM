...
<div class="ff-g16">
  ...
  <table border="0" width="100%" cellpadding="2" class="hovertable">
		<tr>
		  <td colspan="10"><b><font face="Arial" size="4">TEMPAHAN DILULUSKAN</font></b></td>
		  </tr>
		<tr>
		...
		<?php
			$id_fasiliti ='';
			$sql = mysql_query("select * from add_fasiliti where status_approve = 2  ORDER BY id");
...
$bilhariasrama=($ifasrama == 'Asrama' ? " X ".$row['bil_bilik']." bilik" : "<br>Dalam ".$row['jum_hari']." hari/malam");
		?>
				<tr bgcolor="#CCFFFF">
					...
					<td width="5%" valign="top" align="center"><font face="Arial" size="2"><a rel="facebox" href="modifyindex.php?act=tolak&id=<?php echo $row['id']; ?>&no_tempahan=<?php echo $row['no_tempahan']; ?>">
              Tolak</a><br>
				  </font></td>
			  </tr>
		 <?php	
...
if(!empty($_GET)){
	...
	elseif($act=="tolak"){
		$up = mysql_query("update add_fasiliti set status_approve = '3' where id = '$id' AND no_tempahan = '$no_tempahan'");
		if($up){
			header("location:belum_lulus.php");	
		}
	}
