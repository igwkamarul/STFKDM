...
<div class="ff-g16">
  ...
  <table border="0" width="100%" cellpadding="2" class="hovertable">
		<tr>
		  <td colspan="10"><b><font face="Arial" size="4">TEMPAHAN DIGAGALKAN</font></b></td>
		  </tr>
		<tr>
		...
		<?php
			...
			$id_fasiliti ='';
			$sql = mysql_query("select * from add_fasiliti where status_approve = 3  ORDER BY id");
			$rows = mysql_num_rows($sql);
			if($rows >0){
			while ($row = mysql_fetch_array($sql))
			{  
			$hargafasiliti=0;$harga=0;$grand_total=0;$idexist='';
			$sql2 = mysql_query("select * from add_fasiliti where id ='".$row['id']."'  AND status_approve = 3  ORDER BY id");
